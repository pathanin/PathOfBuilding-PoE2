describe("TreeTab", function()
	...
	describe("CopyTree", function()
		...
		it("Copies allocNodes, weaponSets, and class info via undo state", function()
			-- Get an existing allocated node (class start node is always allocated)
			local classStartNodeId = build.treeTab.specList[1].curClass.startNodeId
			local classStartNode = build.treeTab.specList[1].allocNodes[classStartNodeId]

			-- Set allocMode on the node
			classStartNode.allocMode = 1

			-- Set class/ascendancy
			build.treeTab.specList[1].curClassId = 1
			build.treeTab.specList[1].curAscendClassId = 1

			-- Add a hash override
			build.treeTab.specList[1].hashOverrides = { [100] = { isAttribute = true, dn = "Strength" } }

			local newSpec = build.treeTab:CopyTree(1, "Copy Test")

			-- weaponSets should be copied via allocMode on nodes
			assert.are.equals(1, newSpec.allocNodes[classStartNodeId].allocMode)

			-- class info should be copied
			assert.are.equals(build.treeTab.specList[1].curClassId, newSpec.curClassId)
			assert.are.equals(build.treeTab.specList[1].curAscendClassId, newSpec.curAscendClassId)

			-- hashOverrides should be deep copied
